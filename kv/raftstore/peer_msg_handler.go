...
func (d *peerMsgHandler) processRequest(entry *eraftpb.Entry, msg *raft_cmdpb.RaftCmdRequest, wb *engine_util.WriteBatch) *engine_util.WriteBatch {
	...
	d.handleProposal(entry, func(p *proposal) {
		...
		switch req.CmdType {
		case raft_cmdpb.CmdType_Get:
			//因为为了保证读到的数据是准确的，所以writebatch到了读部分需要进行一次刷新
...
func (d *peerMsgHandler) processAdminRequest(entry *eraftpb.Entry, msg *raft_cmdpb.RaftCmdRequest, wb *engine_util.WriteBatch) {
	...
	case raft_cmdpb.AdminCmdType_CompactLog:
		...
		if compactLog.CompactIndex >= applySt.TruncatedState.Index {
			...
			d.Send(d.ctx.trans, append([]pb.Message{}, pb.Message{
				MsgType: pb.MessageType_MsgCompactLog,
				Term:    applySt.TruncatedState.Term,
				Index:   applySt.TruncatedState.Index,
				From:    d.peer.Meta.Id,
				To:      d.peer.Meta.Id,
			}))
		}
	case raft_cmdpb.AdminCmdType_Split:
		...
		// log.Infof("delete region item start:%s end:%s", region.StartKey, region.EndKey)
		...
		// log.Infof("add region item start:%s end:%s", region.StartKey, region.EndKey)
		// log.Infof("add region item start:%s end:%s", newRegion.StartKey, newRegion.EndKey)
		storeMeta.Unlock()
...
func (d *peerMsgHandler) processConfChange(entry *eraftpb.Entry, cc *eraftpb.ConfChange, wb *engine_util.WriteBatch) {
	...
	case eraftpb.ConfChangeType_RemoveNode:
		...
		n := searchRegionPeer(region, cc.NodeId)
		// if n == len(region.Peers) means it can't find relative peer.
		if n < len(region.Peers) {
...
func (d *peerMsgHandler) process(entry *eraftpb.Entry, wb *engine_util.WriteBatch) *engine_util.WriteBatch {
	...
	//如果是admin进行处理,compact,split,changepeer,leaderTransfer
...
func (d *peerMsgHandler) HandleRaftReady() {
	...
	if d.RaftGroup.HasReady() {
		...
		//result不等于nil代表需要apply snapshot
		if result != nil {
...
func (d *peerMsgHandler) HandleMsg(msg message.Msg) {
	...
	case message.MsgTypeStart:
		d.startTicker()
		log.Infof("%s begin to tick.", d.Tag)
	}
}
...
func (d *peerMsgHandler) proposeRequest(msg *raft_cmdpb.RaftCmdRequest, cb *message.Callback) {
	...
	if len(msg.Requests) > 1 {
		panic("unexpected multi request.")
	}
	req := msg.Requests[0]
	key := getRequestKey(req)
	...
	// log1.Printf("type:%s,key:%s", req.CmdType, key)
...
func (d *peerMsgHandler) proposeAdminRequest(msg *raft_cmdpb.RaftCmdRequest, cb *message.Callback) {
	...
	case raft_cmdpb.AdminCmdType_ChangePeer:
		...
		//if there exists confindex not applied, refuse request
		if d.RaftGroup.Raft.PendingConfIndex > d.peerStorage.AppliedIndex() {
			cb.Done(&raft_cmdpb.RaftCmdResponse{
				Header: &raft_cmdpb.RaftResponseHeader{Error: &errorpb.Error{Message: "pending conf not applied"}},
				AdminResponse: &raft_cmdpb.AdminResponse{
					CmdType:    raft_cmdpb.AdminCmdType_ChangePeer,
					ChangePeer: &raft_cmdpb.ChangePeerResponse{},
				},
